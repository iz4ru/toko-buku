...
@section('title', 'Papery | Kelola Data Kasir')
@section('content')
    ...
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg w-full mt-14">
        ...
        <div class="flex gap-4 justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-600">Kelola Data Kasir</h1>
                <p class="text-gray-400">Kelola data kasir untuk menambah, mengubah, dan menghapus akun kasir.</p>
            </div>
        </div>
                        ...
                        <tr class="border-b hover:bg-gray-50 transition-all">
                            ...
                            <td class="px-6 py-4">
                                <img class="w-8 h-8 rounded-full"
                                    src="https://ui-avatars.com/api/?name={{ urlencode($cashier->name) }}&background=1779FF&color=fff"
                                    alt="user photo">
                            </td>
                            <td class="px-6 py-4 font-semibold text-gray-600">{{ $cashier->name }}</td>
                            <td class="px-6 py-4 font-semibold text-gray-600">{{ $cashier->email }}</td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.employee.edit', $cashier->id) }}"
                                        class="text-[#1776FC] hover:underline font-medium focus:outline-none">Edit</a>
                                    <p class="font-bold text-gray-300">|</p>

                                    <form action="{{ route('admin.employee.destroy', $cashier->id) }}" method="POST"
                                        class="delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            class="delete-btn cursor-pointer text-red-500 hover:underline"
                                            data-cashier="{{ $cashier->name }}">Hapus</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
        ...
        <script>
            ...
            document.addEventListener('click', function(e) {
                if (e.target && e.target.classList.contains('delete-btn')) {
                    ...
                    const cashierName = button.getAttribute('data-cashier');

                    Swal.fire({
                        title: `Yakin hapus pengguna "${cashierName}"?`,
                        text: 'Data user ini akan hilang secara permanen.',
                        icon: 'warning',
                        iconColor: '#EF4444',
